...
class NfcoreTemplate {
    ...
    public static void checkConfigProvided(workflow, log) {
        if (workflow.profile == 'standard' && workflow.configFiles.size() <= 1) {
            log.warn "[$workflow.manifest.name] You are attempting to run the pipeline without any custom configuration!\n\n" +
                    ...
                    "   (1) Using an existing pipeline profile e.g. `-profile docker`\n" +
    ...
    //
    // Submit error and exit
    //
    public static void workflowError(params) {
        def id = ''
        if (params.sample_id) {
            id = params.sample_id
            NfcoreTemplate.submit(params.submit_url, id, 'error')
        } else if (params.analysis_id) {
            id = params.analysis_id
            NfcoreTemplate.submitAnalysis(params.submit_url, id, 'error')
        } else {
            id = params.batch_id
            NfcoreTemplate.submit_batch(params.submit_url, id, 'error')
        }
    ...
    // Submit result
    //
    public static void submit(url, sample_id, status) {
        def http = new HTTPBuilder(url)
        http.request(POST) {
            uri.path = '/'
            body = ['sampleId': sample_id, 'status': status]
            requestContentType = JSON

            response.success = { resp ->
                println "Submit result success! ${resp.status}"
            }

            response.failure = { resp ->
                println "Submit result failed with status ${resp.status}"
            }
        }
    }
}
